...
<div class="tab-pane fade show" id="draft" role="tabpanel" aria-labelledby="draft-tab">
    @foreach($offersCollect as $collect)
        @if($collect->first()->offer_status == 'accept')
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-3 pt-4">
                            <div class="d-flex justify-content-center">
                                ...
                                <form class="w-75 px-1" method="POST" action="{{route('publishToNew', $collect->first()->application_id)}}">                                                            @csrf
                                    <button disabled class="btn btn-danger mt-2 w-100" type="submit">
                                        Отозвать
                                    </button>
                                </form>

                                {{-- Modal accept acceptOffer --}}
                                <form class="w-75 px-1" method="POST" action="{{route('acceptApplication', $collect->first()->application_id)}}" style="justify-content: center; align-items: center;">
                                    @csrf
                                    <div>
                                        <button type="button" data-toggle="modal" data-target="#acceptApplication-{{$collect->first()->offer_id}}" class="btn btn-success btn mt-2 w-100 rounded">
                                            Принять
                                        </button>
                                    </div>
                                    <!-- Modal -->
