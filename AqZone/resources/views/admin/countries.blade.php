...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mt-5 card p-5" style="background-color: rgba(214,214,214,0.39)">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLongTitle">Создать страну</h5>
                        ...
                        <div class="modal-body">
                            <form method="POST" action="{{route('createCountry')}}">
                                @csrf
                                <span class="mt-3 text-secondary">Называние страны </span>
                                <input name="countryName" class="form-control mb-3">
        ...
        <div class="tab-content">
            <table class="table align-middle mb-0 bg-white" style="border-radius: 20px">
                ...
                @foreach($countries as $country)
                    <tr>
                        <td>
                            <p class="fw-normal mb-1">{{$country->id}}</p>
                        </td>
                        <td>
                            <p class="fw-normal mb-1">{{$country->countryName}}</p>
                        </td>
                        <td>
                            <p class="fw-normal mb-1">{{$country->created_at}}</p>
                        </td>
                        <td>
                            <p class="fw-normal mb-1">{{$country->updated_at}}</p>
                        </td>
                        <td>
                            <button type="button" class="btn btn-outline-success btn-sm" data-toggle="modal" data-target="#cat-{{$country->id}}">
                                Изменить
                            </button>
                            {{--  modal to edi aids--}}
                            <div class="modal fade" id="cat-{{$country->id}}}" tabindex="-1" role="dialog" aria-labelledby="cat-{{$country->id}}}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
    ...
    <div class="d-flex justify-content-center align-content-center mt-3">
        <a href="{{ $countries->previousPageUrl() }}"><button class="btn btn-outline-primary btn-md" type="button">Назад</button></a>
        <span class="mx-3">
            {{ $countries->withQueryString()->links() }}
        </span>
        <a href={{ $countries->nextPageUrl()  }}>   <button class="btn btn-outline-primary btn-md ml-3" type="button">Следущий</button></a>
    </div>
</main>
</div>
</div>
