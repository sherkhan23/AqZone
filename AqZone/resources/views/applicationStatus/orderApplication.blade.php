...
<div class="tab-pane fade show" id="order" role="tabpanel" aria-labelledby="order-tab">
    @foreach($viewCollect as $collect)
        @if($collect->first()->user_id == auth()->id())
            @if($collect->first()->app_status == 'accept')
                <div class="card shadow-sm border-5 rounded-3 mt-2" style="border: 1px solid gray">
                    <div class="card-body">
                        ...
                        <div class="d-flex justify-content-center">
                            ...
                            <form class="w-100 p-3" method="POST" action="{{route('acceptOfferToDraft', $offerCollect->first()->offer_id)}}">                                                            @csrf
                                <button class="btn btn-outline-danger btn-sm mt-2 w-100 ml-1" type="submit">
                                    Отмена
                                </button>
                            </form>

                            <form class="w-100 p-3" method="POST" action="{{route('acceptOfferToDraft', $offerCollect->first()->offer_id)}}">                                                            @csrf
                                <button class="btn btn-outline-success btn-sm mt-2 w-100 ml-1" type="submit">
                                    Выпелнено
                                </button>
                            </form>
